...
const char * const BITCOIN_CONF_FILENAME = "bash.conf";
const char * const BITCOIN_PID_FILENAME = "bashd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "bash" is a composite category enabling all Bash-related debug output
            if(ptrCategory->count(string("bash"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BashCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BashCore
    // Mac: ~/Library/Application Support/BashCore
    // Unix: ~/.bashcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BashCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BashCore";
#else
    // Unix
    return pathRet / ".bashcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bash.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bash.conf
