 ...
 *
 *   // network
 *   CBashNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.bash.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CBashNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
// // can be under: BashNetwork
// //   -- signature requirements : Key1(User)
// class CBashNetworkVariable : public CGovernanceObject
// {
// private:
